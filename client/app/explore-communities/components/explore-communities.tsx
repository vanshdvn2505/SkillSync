...
import { GET_COMMUNITIES } from "@/graphql/queries/communityQueries"
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {communities.map((community: Community) => (
        <Card key={community.id} className="flex flex-col bg-foreground text-primary-foreground border-none shadow-lg">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <div className="flex items-center mt-1">
                  ...
                  <span className="text-sm text-muted">{community.rating.toFixed(1) == "0.0" ? "Not Rated": community.rating.toFixed(1)}</span>
                </div>
              </div>
            </div>
          </CardHeader>
          ...
          <CardFooter>
            <Link href={`/community/${community.id}/${community.isJoined}`} className="w-full">
